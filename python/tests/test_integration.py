...
class TestDataLoaderIntegration:
    ...
    def sample_csv_file(self, tmp_path: Path) -> Path:
        ...
        df = pd.DataFrame({
            ...
            'temperature': 20 + 5 * np.sin(np.linspace(0, 10, n_rows)) + np.random.randn(n_rows) * 0.5,
            'pressure': 100 + 10 * np.cos(np.linspace(0, 10, n_rows)) + np.random.randn(n_rows) * 1.0,
    ...
    def multiple_csv_files(self, tmp_path: Path) -> list[str]:
        ...
        for i in range(3):
            df = pd.DataFrame({
                'timestamp': pd.date_range(f'2024-01-0{i+1}', periods=100, freq='1min'),
...
class TestEndToEndWorkflows:
    ...
    def workflow_data(self, tmp_path: Path) -> Path:
        ...
        df = pd.DataFrame({
            ...
            'temperature': 20 + 5 * np.sin(2 * np.pi * t) + np.random.randn(n) * 0.5,
